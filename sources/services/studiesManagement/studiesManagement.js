	...
	_init() {
		this._accordion.attachEvent("onAfterExpand", (id) => {
			...
			ajaxActions.getStudy(accordionItem.config.elementId).then((item) => {
				accordionItem.getChildViews()[0].addView(infoPanel.createInfoPanel(item));
				accordionItem.getChildViews()[0].addView(annotatorsPanel.createAnnotatorsPanel(item));
				accordionItem.getChildViews()[0].addView(imagesPanel.createImagesPanel(item));
				// add condition if studyAdmin, then add action panel
				if (authService.isStudyAdmin()) {
					accordionItem.getChildViews()[0].addView(actionPanel.createActionsPanel(item, this));
				}
