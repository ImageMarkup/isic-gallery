...
export default class ImagesSelectionWindow extends JetView {
				...
				markCheckbox: {
					...
					on: {
						onChange(value) {
							...
							if (value) {
								selectedImages.addToSelectedInAddNewImagePopup({
									id: item._id,
									name: item.name
								});
							} else {
							...
							dataview.updateItem(this.config.$masterId, item);
						...
						cols: [
							...
							{width: 7},
							{
								...
								template: () => {
									...
									let text = `<span class='gallery-select-all-images-on-all-pages'> You have been selected ${this.getImagesCount(selectedImagesCount)}`;
	...
	reload(offsetSource, limitSource) {
		let page = this.pager.data.page;
		let limit = limitSource || this.pager.data.size;
		let oldOffset = page * limit;
		let offset = offsetSource || oldOffset;
